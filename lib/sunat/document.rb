...
#     
    ...
    def build_pdf_footer(pdf)
        ...
        pdf.text "Observaciones: " + $lcObserva
         pdf.text "  "
          pdf.text "  "


      pdf.bounding_box([0, 270], :width => 535  ,height: 270 ) do
      ...
      pdf.stroke_horizontal_rule
       ...
       if $lcDocumentIdFactura == 2
         ...
         row <<{:content =>"CUOTAS",:rowspan => 4,:border_width => 0} 
        ...
        table_content << row
         ...
         if $lcMonedaValor  == "PEN"
          ...
          facturas = Factura.find($lcGlobalNumero)


         row = []
         row << "01"
         row << facturas.fecha_cuota1.strftime("%d/%m/%Y")       
         row << @moneda_valor   

         row << facturas.importe_cuota1   
         ...
         row << "02"
         row << facturas.fecha_cuota2.strftime("%d/%m/%Y")       
         row << @moneda_valor   

         row << facturas.importe_cuota2

         table_content << row
          row = []

         row << "03"
         row << facturas.fecha_cuota3.strftime("%d/%m/%Y")       
         row << @moneda_valor   

         row << facturas.importe_cuota3
    ...
    if $lcDiasPago !=  0 
     ...
     pdf.table([["       ", {:image => image_path,:position => :center,  vposition: :center}  , "
  ...
  OPERACION SUJETA AL SISTEMA DE PAGO DE OBLIGACIONES TRIBUTARIAS CONEL GOBIERNO CENTRAL D-LEG.NRO 940
  Representacion impresa de la factura electronica https://www.nubefact.com/20xxxxx  "]],:cell_style => { :border_width => 0 } )
