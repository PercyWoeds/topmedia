  ...
  def data(items = 0, currency = 'PEN')
    ...
    puts "xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx"
       ...
       if @invoice.servicio == "true"
        ...
        $lcRuc          = @invoice.medio.ruc
        $lcFormapago    = @invoice.payment.descrip
        $lcTd           = @invoice.document.descripshort
        
        
         legal_name_spaces = @invoice.medio.descrip.lstrip    
        
        if legal_name_spaces == nil
            $lcLegalName    = legal_name_spaces
        else
            $lcLegalName = @invoice.medio.descrip.lstrip    
        end
        $lcDirCli       = @invoice.medio.address1
        $lcDisCli       = ""
        $lcProv         = ""
        $lcDep          = ""
        $lcPlaca        = ""  
