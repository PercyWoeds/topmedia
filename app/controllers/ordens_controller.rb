...
#        if(params[:search] and params[:search] != "")
 #         @ordens = Orden.where(["company_id = ? and (code iLIKE ?)", @company.id ,"%" + params[:search] + "%" ]).order('fecha DESC').paginate(:page => params[:page])
        if(params[:q] and params[:q] != "")
          ...
          @ordens = Orden.paginate(:page => params[:page]).order('code desc').where(["company_id = ? AND code ilike ?", @company.id, query])
        else
          @ordens = Orden.where(["company_id = ?",@company.id ]).order('code desc ').paginate(:page => params[:page])
